...
class IntpEnv(gym.Env):
    ...
    # Map the action (element of {0,1,2,3}) to the direction we walk in
        ...
        if success == 1:
            f = (self.F_min*q*np.max(self.protocols[0])+self.F_max*(1-q)*np.min(self.protocols[0]))/(q*np.max(self.protocols[0])+(1-q)*np.min(self.protocols[0]))
